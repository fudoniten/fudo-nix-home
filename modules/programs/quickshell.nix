    ...
    text = ''
      ...
      usage() {
        printf '%s\n' \
          ...
          '  theme     Refresh Theme.qml by hand (normally automatic every switch)' \
...
in {
    ...
    dev = {
      enable = mkOption {
        ...
        description = ''
          ...
          The trade-off is that the component QML (Bar.qml, shell.qml, ...)
          is then yours, not Nix's -- edit freely, but it is no longer
          reproducible and won't update on its own. Theme.qml is the one
          exception: it is regenerated from stylix.base16Scheme and
          overwritten on every `home-manager switch` regardless, since
          nothing about it is meant to be hand-edited. Turn `dev.enable` off
          entirely to go back to the fully reproducible, read-only config.
        '';
      };
  ...
  config = mkIf cfg.enable {
    ...
    # stylix.base16Scheme. Seed-once for it would mean every fix to the
    # generator above needs a manual `fudo-quickshell theme` to actually
    # reach an existing dev install, which is exactly what let a real bug
    # here (a missing `import QtQuick`, fixed in the generator) keep
